...
class ChatOutput(QTextBrowser):
    ...
    # ------------------------------------------------------------------
    # 排队气泡三方法（0634 计划 D4，新轨 ChatTranscriptView 同构接口）：
    # QTextBrowser 无块级控件，句柄仅为不透明令牌（panel 队列簿记用）——
    # promote/discard 操作时向前查找「整块文本恰为排队中」的徽标块定位
    # 首个排队段（FIFO 语义下首个即队首，天然正确；position/光标簿记
    # 均会被文档末追加推走，探针实证不可用）。
    # 已知取舍：纯文本文档无法内嵌 ×删钮，旧轨单条删除不可达——队列
    # 清空走停止钮（文本退回输入框），转正即正常气泡（冻结轨新功能
    # 上屏接口，非冻结违约）。
    # ------------------------------------------------------------------
    def append_queued_user_message(self, content: str, images: list | None = None):
        """排队气泡上屏：用户气泡卡 + 卡下「排队中」灰徽标行；返回令牌句柄。"""
        self._flush_stream()
        body = self._linkify_html(_html_escape(content)).replace("\n", "<br>")
        for img in images or []:
            uri = Path(img["path"]).resolve().as_uri()
        ...
        cursor.insertText("\n排队中", fmt)
        self._scroll_to_bottom()
        return object()  # 不透明令牌：定位由 _first_queued_segment 现算

    def promote_queued(self, handle) -> None:
        """出队发送：删除队首徽标行转正式气泡（气泡卡本体保留原位）。"""
        segment = self._first_queued_segment()
        if segment is None:
            return
        _start, badge_start, badge_end = segment
        cursor = QTextCursor(self.document())
        cursor.setPosition(badge_start)
        cursor.setPosition(badge_end, QTextCursor.MoveMode.KeepAnchor)
        cursor.removeSelectedText()
        self._scroll_to_bottom()

    def discard_queued(self, handle) -> None:
        """停止清空：队首气泡卡 + 徽标行整段移除（逐条调用即整队清空）。"""
        segment = self._first_queued_segment()
        if segment is None:
            return
        start, _badge_start, badge_end = segment
        cursor = QTextCursor(self.document())
    ...
    def _first_queued_segment(self) -> tuple[int, int, int] | None:
        """首个排队段定位：返回 (段起始, 徽标起始, 徽标结束)；无则 None。

        徽标识别 = 整块文本恰为「排队中」的块（用户消息经气泡卡带
        「我：」前缀，不可能整块等值，防误命中）；段起始 = 徽标前表格
        框架位（cell(0,0) 首位置 -1）。
        """
        pos = 0
        while True:
            found = self.document().find("排队中", pos)
            if found.isNull():
                return None
            if found.block().text() == "排队中":
                break
            pos = found.position()
        badge_end = found.position()
        cursor = QTextCursor(self.document())
        cursor.setPosition(found.selectionStart())
        cursor.movePosition(QTextCursor.MoveOperation.StartOfBlock)
        badge_start = cursor.position() - 1  # 含块前换行
        # 段起始 = 徽标前表格框架位：表格与徽标块之间隔一个表格自带的
        # 空块（探针实证），向前跳过空块至表格单元内取 cell(0,0) 首位置 -1
        start = badge_start  # 防御回退：结构异常时只删徽标
        probe = QTextCursor(cursor)
        while probe.movePosition(QTextCursor.MoveOperation.PreviousBlock):
            if probe.block().text():
                break
        table = probe.currentTable()
        if table is not None:
            start = table.cellAt(0, 0).firstPosition() - 1
        return (start, badge_start, badge_end)
