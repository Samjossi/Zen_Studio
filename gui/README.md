...
| `gui/panels/chat/` | 聊天面板子包（左栏）：`panel.py` 装配（含底行：模型选择按钮 + 发送/停止双态按钮）/ `output.py` 输出区 / `input.py` 输入框（文件拖入 → `@路径` 引用）/ `model_bar.py` 模型版本选择按钮（纯视图）/ `worker.py` 流式线程 / `permission_dialog.py` ACP 工具审批对话框 |
...
| `ModelBar` | 输入区底行左端双按钮（纯视图，每标签实例）：模型（Kimi CLI / Kimi ACP，不可用时禁用）+ 版本（模型别名联动刷新），框体恒显「模型」「版本」标签（当前值归 tooltip 全名），点击弹 InstantPopup 菜单——按内容加宽显示全文、当前项 ✓ 勾选；切换下次请求生效，任一标签发送中全标签锁定；选择与写盘归 ChatTabs 状态层（`model_backend` / `model_version`），启动时恢复 |
