...
| `llm/providers/kimi_acp.py` | `KimiAcpLLM`：Kimi ACP 后端（长驻 `kimi acp` 子进程，复用 `AcpConnection`；token 级流式、思维链可见、审批反向请求路由；上下文用量走 wire.jsonl 落盘记录读取，source="transcript"，2026-0731-1454；轮次内 `poll_usage()` 尾部轮询实时刷新，2026-08-02，文档/修改记录/2026-0802-0117；子代理 wire 旁路 `_WireSidecar`：轮次内发现 `agents/agent-N/` 新增目录、增量解析 wire.jsonl 合成层级 tid 帧与 conn 双源汇合——子代理内部活动嵌套显示，设置键 `kimi_wire_sidecar` 默认开，2026-08-13，work plans/2026-0813-1919 计划 T4，格式假设见 维护手册/后端CLI工具维护说明手册.md） |
