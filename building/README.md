...
Zen Studio 全部打包构建活动的专属目录（2026-07-25 收编，见 文档/修改记录/2026-0725-1053 计划）。构建链：**PyInstaller onedir → AppDir 组装 → appimagetool → AppImage**，Linux x86_64 单平台。
...
- 🧹 构建脚本前置自查自动清洗 `LD_LIBRARY_PATH`/`QT_PLUGIN_PATH` 中的 AppImage 挂载点路径（`/.mount_*`）——在本 IDE AppImage 集成终端内直接构建是受支持的（背景见 `文档/审计报告/2026-0811-0841_集成终端环境变量污染问题报告_ZenStudio.md`：污染曾致 PyInstaller 静默剔除全部 Qt 插件）；
