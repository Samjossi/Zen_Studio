...
Zen Studio 的设计哲学是 **Agent 驱动**：代码修改一律经 AI agent 落盘，IDE 本体承担「看、问、跑」三件事——
...
| 特性 | 说明 |
|:---|:---|
| AI 聊天面板 | 多标签会话（上限 4，标签间 provider 完全隔离可并行）；对话区双轨渲染——KiloCode 式卡片折叠（默认，工具/思维链/diff/TODO 全族卡，开合状态记忆）与经典流式可切换；token 级流式 + 思维链可见（ACP 后端）；图片附件多模态发送（粘贴/拖入/选择，缩略图 chip 行，能力外后端自动退化 `@路径` 引用）；上下文 token 用量徽章（kimi 轮次内实时刷新）；发送/停止双态按钮；拖入文件插 `@相对路径` 引用；ACP 工具审批模态框（允许一次/始终允许/拒绝） |
...
| Git 集成 | 文件树状态着色（M/U/D）、查看器差异徽标（`+a -b`）、右栏下变更面板（VS Code SCM 简化版）、状态栏差异统计；提交历史图弹窗（结构化列表：自绘图形列 + refs 胶囊徽标 + 相对·绝对双时间，ANSI 彩色降级形态兜底）；事件驱动刷新（窗口激活 / 外部重载 / 手动） |
| 多开工作区 | 一进程绑定一工作区根；「在新窗口打开文件夹」起独立进程，窗口状态按工作区哈希分别持久化（VS Code `workspaceStorage` 语义） |
| 主题体系 | 多主题注册表（云白/暖米/晴空/薄荷/暗色），自带思源黑体 / 更纱黑体 / Noto Color Emoji 字体族，主题令牌化治理 |
...
| `参考代码/` | 五个参考项目（gitignored 隔离分发，仅供阅读借鉴） |
...
前置条件：至少安装五后台之一的 agent CLI 并完成其登录（如 Kimi Code CLI ≥ 0.2.0 执行 `kimi login`，OAuth 凭证由 CLI 自管，IDE 不接触）。
...
| 配置 | 位置 | 说明 |
|:---|:---|:---|
| 应用偏好 | `config/settings.json` | 主题 / 字号 / 对话区渲染（cards 默认 / classic）/ 模型选择（按接口记忆的 `model_versions` 表）；多进程并发经 flock 文件锁 + 原子写治理 |
...
Zen Studio 站在巨人的肩膀上。感谢以下开源项目，它们的设计为本项目提供了重要蓝本：

| 参考项目 | 借鉴点 |
|:---|:---|
| Eclipse Theia | `LanguageModel` 协议抽象、面板装配范式（仅设计参考，未复制实质代码，审计见 `文档/审计报告/`） |
| Visual Studio Code | `workspaceStorage` 窗口状态语义、SCM 变更面板形态 |
...
项目整体采用 **MIT** 协议（结论见 `文档/审计报告/` theia 实质代码审计报告 §4.2），根目录 `LICENSE`（MIT）与 `NOTICE`（致谢）均已补入。
