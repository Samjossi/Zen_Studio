...
2026-08-12 自 `.temp/frame_archive/` 迁入根目录 `帧存档/`（该目录曾计划
gitignore 豁免入库，豁免行在 .gitignore 重写中丢失；改为独立跟踪目录，
杜绝反规则脆弱性）。2026-08-13 再自根目录迁入 `文档/帧存档/`（根目录
观感碍事，归档文档专区集中安置）。此两日期前的计划文档/审计报告中引用
的 `.temp/frame_archive/` 与根级 `帧存档/` 路径均即本目录。
...
- 命名：`{工具}_{后端或场景}_{YYYYMMDD_HHMMSS}.json`，关联截图同名
  `.png`。
- 引用纪律：代码注释引用蓝本时写本目录相对路径（`文档/帧存档/xxx.json`）。
